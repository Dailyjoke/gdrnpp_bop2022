...
class XyzGen(object):
    def __init__(self, split="train", scene="all"):
        ...
        # if split != "train":
        #     raise ValueError(f"Only train split supported, got {split}")
        ...
        # self.sel_scene_ids = scenes if scene == "all" else [int(scene)]
        
        self.split = split
        self.scene = scene
        print("split:", split, "selected scene ids:", self.sel_scene_ids)

        self.data_root = data_root
        self.renderer = None
        
        # 載入 camera.json
        cam_path = osp.join(PROJ_ROOT, "datasets/BOP_DATASETS/lmo/camera.json")  
    ...
    def get_renderer(self):
        if self.renderer is None:
            ...
            self.renderer = Renderer(
                ...
                vertex_tmp_store_folder=osp.join(PROJ_ROOT, ".cache"),
                vertex_scale=0.001,
                height=IM_H,
                width=IM_W,
                near=near,
        ...
        for scene_id in tqdm(sel_scene_ids, postfix=f"{split}_{scene}"):
            ...
            for str_im_id in tqdm(gt_dict, postfix=f"{scene_id}"):
                ...
                for anno_i, anno in enumerate(gt_dict[str_im_id]):
                    ...
                    bgr_gl, depth_gl = self.get_renderer().render(render_obj_id-1, IM_W, IM_H, K, R, t, near, far)
