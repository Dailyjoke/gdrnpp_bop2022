...
def main():
    ...
    for i in tqdm(range(num_train_imgs)):
        ...
        while True:
            ...
            img_path = osp.join(
                sixd_train_path,
                "{:06d}".format(rand_obj_id + 1),
                "rgb",
                "{:06d}.png".format(rand_view_id),
            )
            ...
            M = cv2.getRotationMatrix2D(
                (int(rand_img.shape[1] / 2), int(rand_img.shape[0] / 2)),
                rot_angle,
                1,
            )
            ...
            mask_path = osp.join(
                ...
                "{:06d}".format(rand_obj_id + 1),
                "mask",
                "{:06d}_{:06d}.png".format(rand_view_id, 0),
            )
            ...
            scaled_mask = cv2.resize(
                mask.astype(np.int32).reshape(orig_H, orig_W, 1),
                (new_W, new_H),
