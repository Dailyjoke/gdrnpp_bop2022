...
class GDRN_Dstream_DoubleMask(nn.Module):
    def __init__(self, cfg, backbone, depth_backbone, geo_head_net, neck=None, pnp_net=None, fuse_net=None):
        super().__init__()
    ...
    def forward(
        ...
        if net_cfg.FUSE_RGBD_TYPE == "cat":
            conv_feat = torch.cat([rgb_feat, depth_feat], dim=1)
        elif net_cfg.FUSE_RGBD_TYPE == "add":
            assert rgb_feat.shape == depth_feat.shape
            conv_feat = rgb_feat + depth_feat
        elif net_cfg.FUSE_RGBD_TYPE == "fuse_net":
            assert self.fuse_net is not None
            conv_feat = self.fuse_net(rgb_feat, depth_feat)
        else:
...
def build_model_optimizer(cfg, is_test=False):
    ...
    # fuse net (fuse rgb branch and depth branch) -------------------'
    if net_cfg.FUSE_RGBD_TYPE == "fuse_net":
        fuse_net, fuse_net_params = get_fuse_net(cfg)
        params_lr_list.extend(fuse_net_params)
    else:
        fuse_net = None
