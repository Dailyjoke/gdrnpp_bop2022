...
class GDRN_CLS2REG(nn.Module):
    def __init__(self, cfg, backbone, geo_head_net, neck=None, pnp_net=None):
        super().__init__()
        assert cfg.MODEL.POSE_NET.NAME == "GDRN_cls2reg", cfg.MODEL.POSE_NET.NAME
    ...
    def train(self, mode=True):
        ...
        super(GDRN_CLS2REG, self).train(mode)
        cfg = self.cfg
    ...
    def forward(
        ...
        if coor_x.shape[1] > 1 and coor_y.shape[1] > 1 and coor_z.shape[1] > 1:
            coor_x_reg = soft_argmax(coor_x, dim=1, keepdim=True)
            coor_y_reg = soft_argmax(coor_y, dim=1, keepdim=True)
            coor_z_reg = soft_argmax(coor_z, dim=1, keepdim=True)
        else:
            raise RuntimeError("rot head should output xyz cls")
        coor_feat = torch.cat([coor_x_reg, coor_y_reg, coor_z_reg], dim=1)  # BCHW
        ...
        if not do_loss:  # test
            ...
            out_dict = {}
            assert (gt_trans is not None) and (gt_trans_ratio is not None) and (gt_region is not None)
    ...
    def gdrn_loss(
        ...
        if not g_head_cfg.FREEZE:
            ...
            if xyz_loss_type in ["L1", "CE_coor/L1"]:
                assert gt_xyz is not None
            ...
            if xyz_loss_type in ["CE_coor", "CE_coor/L1"]:
                assert gt_xyz_bin is not None
                gt_xyz_bin = gt_xyz_bin.long()
                loss_func = CrossEntropyHeatmapLoss(reduction="sum", weight=None)  # g_head_cfg.XYZ_BIN+1
                loss_dict["loss_coor_x_bin"] = loss_func(
                    out_x_bin * gt_mask_xyz[:, None], gt_xyz_bin[:, 0] * gt_mask_xyz.long()
                ) / gt_mask_xyz.sum().float().clamp(min=1.0)
                loss_dict["loss_coor_y_bin"] = loss_func(
                    out_y_bin * gt_mask_xyz[:, None], gt_xyz_bin[:, 1] * gt_mask_xyz.long()
                ) / gt_mask_xyz.sum().float().clamp(min=1.0)
                loss_dict["loss_coor_z_bin"] = loss_func(
                    out_z_bin * gt_mask_xyz[:, None], gt_xyz_bin[:, 2] * gt_mask_xyz.long()
                ) / gt_mask_xyz.sum().float().clamp(min=1.0)
                loss_dict["loss_coor_x_bin"] *= loss_cfg.XYZ_LW
                loss_dict["loss_coor_y_bin"] *= loss_cfg.XYZ_LW
                loss_dict["loss_coor_z_bin"] *= loss_cfg.XYZ_LW
...
def build_model_optimizer(cfg, is_test=False):
    ...
    # build model
    model = GDRN_CLS2REG(cfg, backbone, neck=neck, geo_head_net=geo_head, pnp_net=pnp_net)
    if net_cfg.USE_MTL:
        params_lr_list.append(
            {
                "params": filter(
